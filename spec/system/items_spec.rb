...
def basic_pass(path)
  username = Rails.application.credentials.basic[:user_name]
...
RSpec.describe '商品編集', type: :system do
  ...
  context '商品編集ができるとき' do
    it 'ログインしたユーザーは自分が出品した商品の編集ができる' do
      ...
      # 編集ページへ遷移する
      click_on '商品の編集'
      # すでに投稿済みの内容がフォームに入っている(画像以外、type=fileには値が入らないため。)
      expect(
...
RSpec.describe '商品詳細', type: :system do
  ...
  context 'ログインしてないとき' do
    it 'ログインせずに商品詳細ページへアクセスした場合は、「購入」のリンクが存在しない' do
      # トップページへ行く
      visit root_path
      # 商品のページへアクセスする
      find(:xpath, "//a[@href='/items/#{@item2.id}']").click
      # 商品詳細ページに「購入画面に進む」のボタンがない
